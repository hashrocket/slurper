...
Slurper requires a .slurper.yml file in your home directory. This file contains your Tracker API information and default values for stories.
...
The name, labels and description fields provide slurper with default values for story titles and descriptions if you don't provide them in your stories.slurper file.
...
=== Example stories.slurper
  ...
  ==
  name:
    User Creates Something Else
  description:
    In order to accomplish some other business value
    As a User
    I want to create some other thing
...
Note: the story source file is whitespace-sensitive. Be sure the value for each key is indented with two spaces beneath each key. Also, delineate each story with a double-equals on its own line.
